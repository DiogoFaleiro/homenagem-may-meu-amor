/* Grand finale: casal (silhueta sólida com brilho) -> dissolve em partículas
   -> iniciais -> coração pulsando -> "Te amo para sempre" -> brilho.

   O casal andando é desenhado como silhueta sólida com brilho na borda — essa
   técnica sempre leu bem como "duas pessoas" (já tentamos 3D e partículas
   contínuas pro casal também; nenhum dos dois ficou tão claro quanto isso).
   No momento do abraço, a silhueta se desfaz numa nuvem de partículas — e é aí
   que a estética de poeira do resto do final (iniciais, coração, "Te amo")
   entra, que já funciona bem por ser texto/formas simples e finas.
...
const T_PO = 6.8, T_INICIAIS = 7.6, T_CORACAO = 11.6, T_AMOR = 17.2, T_BRILHO = 21.3;
...
function quadroFinal(agoraMs) {
  ...
  if (t < T_PO + .6) {
    const { chao } = desenharCasal(t);
    const aSil = cl((t - .6) / 1) * (1 - cl((t - (T_PO + .1)) / .5));
  ...
  if (t >= T_PO && fase === 'casal') {
    fase = 'po'; const pts = escolher(amostrarSilhueta(), reduz ? 600 : 1300);
    parts = pts.map(p => ({ x: p.x, y: p.y, ox: p.x, oy: p.y, tx: p.x, ty: p.y, t0: 0, dur: 1, curva: 0, f: Math.random() * 6.28, vx: 0, vy: 0, a: 1 }));
    flash = .35;
  }
  if (t >= T_INICIAIS && fase === 'po') { fase = 'iniciais'; morfar(pontosTexto(parts.length, 'M & D'), 2) }
